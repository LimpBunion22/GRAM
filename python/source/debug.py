...
import copy
from plots import dibujar_vectores
from evaluations import evaluate_base,evaluate_orts

print("---START---\n")
### Define base functions
n_dim = 1 # Data dimensions
n_base_funcs = 45 # First layer neurons
...
for i in range(n_data):
    data_in[i] = (2*i/n_data-1)*np.ones(n_dim)#2*np.random.rand(n_dim) - 1
    data_lab[i] = np.sin(10*np.sum(data_in[i]))

for n_base_funcs in range(5,100,5):
    ...
    for d in range(n_data):
        ...
        network_output[d] = np.sum(first_layer_output*sigma) #sigma
        error += np.abs(data_lab[d] - network_output[d])
        rel_error += np.abs((data_lab[d] - network_output[d])/np.max([1e-8,np.abs(data_lab[d])]))

    error /= n_data
    rel_error *= 100/n_data

    print(str(n_base_funcs)+"--     Test results:")
    print("         Average Absolute error = " + str(error))
    print("         Average Relative error = " + str(rel_error)+"\n")


    plt.figure(figsize=(8, 6))
    plt.plot(data_lab, label="LABELS", color='blue')
    plt.plot(network_output, label="Predicted", color='red')
    plt.title("Neurons = "+str(n_base_funcs))
    plt.grid(True)
    plt.legend()
    plt.show()
